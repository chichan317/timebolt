...
16. ✅ **Fixed-price projects** — `Project.fixedPrice`; `isFixedPrice` in `lib/money.ts`; `resolveRate` returns 0 for them. Project modal Hourly/Fixed toggle; a `fixed` tag in Clients/Reports/Dashboard (excluded from hourly totals, like retainer); invoiced as one flat line.
17. ✅ **Invoice rewrite** — single client (a picker in the invoice, seeded from the Reports client filter via `initialClientId`) with **editable lines** (Description | Amount, add/remove; total recomputes). Lines seed from the client's hourly projects / retainer / fixed-price projects. **Every field is editable on the invoice** (business name/ABN/address/email, invoice no./issued/period, bill-to name/ABN/address/email, payment, notes) — one-off overrides seeded from the profile/client, not saved. No logo/bolt icon. Editing controls + `.invoice-edit-text` inputs render as plain text in `@media print` (empty ones hidden via `:placeholder-shown`).
18. ✅ **No-charge (pro bono) clients** — `Client.nonBillable`; `isNoCharge` in `lib/money.ts`; `resolveRate` returns 0. Client modal gains a third billing mode (Hourly / Retainer / No charge). A `no charge` tag in Clients/Reports/Dashboard (like retainer); excluded from the invoice client picker. The four client kinds (hourly $, retainer, fixed-price, no charge) all read clearly in Reports.
