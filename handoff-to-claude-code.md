...
17. ✅ **Invoice rewrite** — single client (a picker in the invoice, seeded from the Reports client filter via `initialClientId`) with **editable lines** (Description | Amount, add/remove; total recomputes). Lines seed from the client's hourly projects / retainer / fixed-price projects. Header/Bill-to/Payment from the saved profile + client. Editing controls (`.invoice-edit-only`, line inputs) render as plain text in `@media print`.
